...
class Func
{
    ...
    {
        $dirs = ['app/core', 'app/model'];
        foreach ($dirs as $dir) {
            $filename = Cf::getRootPath() . '/' . $dir . '/' . $class . '.php';
    ...
    /**
     * 根据控制器名称和url参数获取链接，需在Cf类实现getPathUrl方法
     * @param string $action
    ...
    public static function U($action = '', $controller = '', $vars = [])
    {
        ...
        $url = self::getHostUrl() . Cf::getPathUrl($action, $controller);
        if (!empty($vars)) {
            $query = '';
    ...
    */
     ...
     * @return array
     */
    public static function fieldsArray($arr, $fields)
    {
        $aFields = explode(',', $fields);
        $ret = [];
        foreach ($aFields as $field) {
            $ret[trim($field)] = $arr[trim($field)];
        }
...
class DbMysqli
{
    ...
    {
        $mysqli = new mysqli($cDbHost, $cDbUser, $cDbPass, $cDbName);
